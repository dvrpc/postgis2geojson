...
parser.add_argument(
    "-d", "--database", dest="database", type=str, required=True, help="The database to connect to"
)
...
parser.add_argument(
    "-p", "--password", dest="password", default="", type=str, help="Password for the database user"
)

parser.add_argument(
    "-P", "--port", dest="port", default="5432", type=str, help="Password for the database user"
)

parser.add_argument(
    "-t", "--table", dest="table", type=str, required=True, help="Database table to query"
)
...
parser.add_argument(
    "-w", "--where", dest="where", type=str, help="Optional WHERE clause to add to the SQL query"
)
...
parser.add_argument(
    "--pretty", dest="pretty", action="store_true", help="Pretty print the output (indent)."
)
...
def get_data():
    ...
    geom_index = colnames.index("geometry")
